...
B가 같은 경우는 어떻게 처리해야 할까? \\(A_i\\)보다 A가 크고 \\(B_i\\)와 B가 같은 쌍들은 P i의 쿼리에서 고려 대상이다. 따라서 B가 같을 땐 A를 오름차순으로 나열해 주면 된다. \\(1<=B<=2*10^9\\)이므로 B를 압축해주어야 한다. B가 같다고 해서 같은 값으로 압축하지 말고 위 관찰을 바탕으로 처리해 주면 된다.